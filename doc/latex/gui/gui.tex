...
es borrado. En caso de problemas de conexión con la base de datos, el
archivo \texttt{userinfo.dat} es eliminado.
...
El comando para ejecutar esta interfaz es: \texttt{gui\_db\_editor}, o
desde la interfaz principal, presionar el botón \emph{Edit database}.
...
\item Menú para insertar, actualizar o eliminar un tipo de
  medición.
\item Menú para insertar o actualizar las rutas de las imágenes.
\end{enumerate}
...
\item Opción para determinar si el sensor es virtual o no.
\item Descripción verbal del sensor.
\item Botón para importar los datos del sensor (solamente archivos
  TXT).
...
seleccionando el archivo por medio del campo (14), y finalmente haciendo
...
duro local. El menú \emph{Generate Image Paths}, permite, dado un
...
Para ejecutar esta interfaz, presionar el botón \emph{Configure
  cameras} desde la interfaz principal o con el comando:
...
\item Botón para guardar la configuración de la cámara. Esta
  ...
  \texttt{capture\_info.xml} en el directorio \texttt{data}.
...
Para ejecutar esta interfaz, presionar el botón \emph{Setup image
  paths} desde la interfaz principal o con el comando:
...
Para ejecutar esta interfaz, presionar el botón \emph{Configure
  rectification} desde la interfaz principal o con el comando:
...
\item Botón para seleccionar un ROI en la imagen.  Este botón
...
\item El usuario tiene la opción de guardar en la base de datos la
  ...
  \emph{Show rectified image with/without grid}.
...
Para ejecutar esta interfaz, presionar el botón \emph{Create new ROI}
desde la interfaz principal o con el comando: \texttt{gui\_roi\_tool}.
...
\item Se mostrarán las posiciones en $u$ (valores reales) si se
  escogió la opción de actualizar y hay información en la base de
  datos, por el contrario, si se escogió la opción de insertar se
  mostrará vacío para que se ingrese la información. El campo que se
  muestra al lado derecho es para visualizar la posición de los puntos
  cuando se está haciendo la marcación con (11). Deben ser mínimo tres
  puntos para poder así cerrar el polígono.
\item Se mostrarán las posiciones en $v$ (valores reales) si se
  escogió la opción de actualizar y hay información en la base de
  datos, por el contrario, si se escogió insertar se mostrará vacío
  para que se ingrese la información. El campo que se muestra al lado
  derecho es para visualizar la posición de los puntos cuando se está
  haciendo la marcación con (11).  Deben ser mínimo tres puntos para
  poder así cerrar el polígono.
...
Para ejecutar esta interfaz, presionar el botón \emph{Manually rectify
  images} desde la interfaz principal o con el comando:
...
\item El usuario tiene la opción de guardar las imágenes generadas por
  el proceso de rectificación en la base de datos y almacenar las
  imágenes en la ruta por defecto del disco especificada con
  anterioridad (esta ruta es escogida por el usuario cuando configura
  por primera vez una estación, y se almacena en
  \texttt{path\_info.xml}), o puede seleccionar una ruta diferente en
  el disco, en caso de que no quiera guardar estas imágenes en la base
  de datos.
\item Botón para seleccionar la ruta del nuevo directorio donde se
  guardarán las imágenes rectificadas. Éste se habilita cuando se
  escoge en (11) la opción de almacenar sólo en el disco.
...
Para ejecutar esta interfaz, presionar el botón \emph{Configure
  fusion} desde la interfaz principal o con el comando:
\texttt{gui\_genfusion}.
...
\item Selección de las cámaras y el orden entre ellas. En la lista
  ...
  ``$\ll$'' borra de la lista de cámaras la cámara seleccionada. Las
  cámaras se deben agregar en el orden en el que se realiza la fusión.
\item Opción para escoger el tipo de la imagen.
...
\item Lista desplegable donde aparecen todos los puntos de control
  ...
  la fusión, junto a su nombre aparece un ``*'' y se dibuja en
...
cámaras. Por defecto, se escoge la última calibración de fusión
almacenada en la base de datos cuyo tiempo de generación sea el más
cercano antes del tiempo de cada conjunto de imágenes, sin embargo, es
posible escoger cualquier otra calibración para fusionar el conjunto
completo de imágenes. En la figura~\ref{fig:fusion} se muestra la
interfaz.
...
Para ejecutar esta interfaz, presionar el botón \emph{Manually merge
  images} desde la interfaz principal o con el comando:
...
\item Se puede seleccionar una de las siguientes tres opciones:
  ``Merge oblique images'' para fusionar imágenes oblicuas (sin
  rectificar), ``Rectify and merge oblique images'' para rectificar
  primero y luego fusionar imágenes oblicuas, y ``Merge rectified
  images'' para fusionar imágenes ya rectificadas.
...
\item Área de texto donde el usuario puede ingresar el tamaño de paso
  ...
  \ldots\}$.
\item El usuario tiene la opción de guardar las imágenes generadas por
  el proceso de fusión en la base de datos y almacenar las imágenes en
  la ruta por defecto del disco especificada con anterioridad (esta
  ruta es escogida por el usuario cuando configura por primera vez una
...
\item Botón para seleccionar la ruta del nuevo directorio donde se
  guardarán las imágenes fusionadas. Este se habilita cuando se escoge
  en (10) la opción de almacenar sólo en el disco.
...
en el archivo XML \texttt{capture\_info.xml} en el directorio
\texttt{data} que utiliza el programa automático de captura.
...
Para ejecutar esta interfaz, presionar el botón \emph{Configure
  capture automatic} desde la interfaz principal o con el comando:
...
\item Tiempo de captura, en segundos. Este campo indica el tiempo en
  el que se capturan imágenes a alta frecuencia (indicada al
  configurar las cámaras) con el fin de producir las imágenes
  \emph{timex} y \emph{variance}.
\item Tipos de imágenes que se desean capturar. Para escoger un tipo
  de imagen, se selecciona de la lista desplegable y con el botón
  $\gg$ se pasa a la lista de tipos seleccionados. Para deseleccionar
  un tipo de imagen, se hace clic sobre el nombre en la lista de la
  derecha, y con el botón $\ll$ se elimina de la lista.
...
\item Botón para generar el automático de captura de imágenes y
  \textit{stacks}. Este botón sólo se habilita cuando se guarda la
  configuración.
\item La configuración del envío de datos contiene varios campos. Este
  ...
  imágenes.
\item Nombre de usuario del servidor SSH.
\item Contraseña del servidor SSH.
...
\item Ruta remota en el servidor en donde se almacenarán los datos
  enviados.
\item Botón para generar el automático de transferencia. Este botón
  sólo se habilita cuando se guarda la configuración.
...
archivos ejecutables\\
 \texttt{run\_auto\_capture\_images.bat} y
...
XML \texttt{processing\_info.xml} en el directorio \texttt{data}.
...
Para ejecutar esta interfaz, presionar el botón \emph{Configure
  processing automatic} desde la interfaz principal o con el comando:
\texttt{gui\_processing}.
...
\item Nombre del servidor FTP.
\item Nombre del usuario para acceder al servidor FTP.
\item Contraseña para acceder al servidor FTP.
...
\item Tipos de las imágenes que serán procesadas.
...
\item Botón para generar el automático de procesamiento. Este botón
  sólo se habilita cuando se guarda la configuración.
...
(en minutos). El tamaño de paso es el tiempo entre las imágenes
consecutivas para la búsqueda, por ejemplo, si la primera imagen se
encuentra a las 06:00 y el tamaño de paso es de 30 minutos, las
siguientes imágenes se buscarán en el tiempo 06:30, 07:00, 07:30, etc;
el error de búsqueda es la desviación en el tiempo de búsqueda, por
ejemplo, si el error es de 3 minutos, y se busca una imagen a las
06:30, entonces las imágenes desde las 06:27 hasta 06:33 son
consideradas (siempre se escoge la imagen con tiempo más cercano al
tiempo de búsqueda). Para ejecutarlo, se debe crear un archivo BAT que
recibe los parámetros. Un ejemplo de archivo BAT para el automático de
...
Para ejecutar esta interfaz, presionar el botón \emph{Configure
  database synchronization automatic} desde la interfaz principal o
con el comando: \texttt{gui\_configure\_sync}.
...
\item Botón para generar el automático de sincronización. Este botón
  sólo se habilita cuando se guarda la configuración.
\item Botón para guardar la configuración en el archivo
...
Para ejecutar esta interfaz, presionar el botón \emph{Configure
  thumbnail generation} desde la interfaz principal o con el comando:
...
\item Ancho en pixeles de las miniaturas. Se recomienda que el ancho
  de la miniatura sea aproximadamente 120 píxeles (el alto de la
  imagen se calcula automáticamente para que la imagen sea
  proporcional al ancho seleccionado).
\item Se debe seleccionar si se desea que las imágenes sean subidas a
  ...
  las opciones ``\textit{Insert to Database}'' y ``\textit{Upload
    hosting}''. Al seleccionar ``\textit{Upload hosting}'' se creará
  un archivo con la información para subir las imágenes al servidor
  web. Para configurar los datos de conexión del \texttt{hosting} y
  subir las imágenes como tal se debe usar la interfaz \emph{Configure
    thumbnail upload}, para ejecutar esta interfaz, presionar el botón
  \emph{Configure thumbnail upload} desde la interfaz principal o con
  el comando: \texttt{gui\_upload\_hosting}.
...
obligatorios). Para ejecutar esta interfaz, presionar el botón
\emph{Configure thumbnail upload} desde la interfaz principal o con el
comando \texttt{gui\_upload\_hosting}:

\begin{enumerate}
\item Lista para seleccionar la estación para la que se quieren subir
  las imágenes miniaturas.
\item Host del servidor FTP que puede ser una dirección IP o un nombre de
  dominio.
