...
como configuración de la captura, procesamiento de las imágenes y
...
detallan a continuación, se despliega una interfaz para hacer el
\textit{login} en la base de datos, con el usuario creado previamente
en la instalación. En la figura~\ref{fig:login} se muestra la ventana
de \textit{login}. Este \textit{login} solamente se hace una vez
mientras dura la sesión de usuario. Cuando se cierra alguna interfaz,
se pregunta al usuario si desea cerrar la sesión o no.
...
gráfica, la conexión con la base de datos caduque, por lo que en ese
...
toda la información generada necesaria para el correcto funcionamiento
del sistema (ver sección~\ref{sec:dbdesign}). Con esta interfaz se
puede insertar, actualizar o eliminar la información de una estación,
una cámara, un GCP, un sensor o un tipo de medición (los cuales son
los parámetros que mide un sensor). Al abrir la interfaz se muestra la
opción de insertar una nueva estación a la base de datos de HORUS. Si
se desea, con el menú de la parte superior se puede cambiar de opción
para actualizar o insertar estaciones, cámaras, GCPs, sensores y tipos
de mediciones. A continuación se explica cada una de las opciones
de la interfaz.
...
\item Alias de la estación, generalmente, una abreviatura del nombre
  ...
  actualizarlo.
\item Elevación de la estación, éste es un valor numérico posiblemente
  con cifras decimales después del (.).
\item Latitud de la estación, éste es un valor numérico posiblemente
  con cifras decimales después del (.).
\item Longitud de la estación, éste es un valor numérico posiblemente
  con cifras decimales después del (.).
...
\item En esta lista se debe seleccionar el GCP que se desea actualizar
  ...
  cada estación, los valores de la coordenada $X$, $Y$ y $Z$ del GCP
  en el sistema coordenado correspondiente, estas tres últimas
  columnas deben ser numéricas. El archivo de Excel solo debe tener
  una hoja de calculo, todas las demás deben ser eliminadas, también
...
los GCPs importados no reemplazarán los existentes haciendo las veces
de actualización, si se desean actualizar se deberán eliminar todos y
...
\item ID para el GCP, éste debe ser único para cada estación, ser
  numérico y entero. Este campo no es posible actualizarlo.
\item Estación asociada al GCP. Este campo no es posible actualizarlo.
\item Nombre del GCP, por lo general es GCP y tres dígitos (e.g.\
  GCP001). Este campo debe ser único para cada estación.
\item Valor de la coordenada $x$ del GCP en el sistema coordenado
  correspondiente. Éste es un valor numérico posiblemente con cifras
...
ejemplo, una boya puede medir altura de ola, período de ola, entre
otros. En la figura~\ref{fig:sensor} se muestra la opción de insertar
...
\item Botón para agregar los datos de la medición a la base de
  ...
  información. Dando clic en este botón se abrirá una ventana en la
  cual se escogerá el archivo que tiene los datos de la
  medición. Este archivo puede tener format MAT o Excel, y no debe
...
Luego de configurar las estaciones y las cámaras para cada estación en
la base de datos, es necesario configurar físicamente las cámaras para
la captura. En esta sección se describe la interfaz utilizada para
llevar a cabo esta tarea, configurar las cámaras conectadas al
computador de captura con los parámetros de captura requeridos. Para
utilizar esta interfaz es necesario disponer del toolbox de
Adquisición de Imágenes de MATLAB.
...
En la sección anterior, se mostró la interfaz para configurar las
cámaras de captura después de haber sido asociadas con las cámaras de
la base de datos. En esta sección, se muestra la interfaz para
configurar la captura como tal, entre las cosas que se configuran se
encuentran las horas del día en que se capturan imágenes y
\textit{stacks}, así como la información para el envío de los
datos. La configuración se guarda en el archivo XML
\texttt{capture\_info.xml} que se utiliza por el programa automático
de captura.
...
\item Tamaño de paso entre envío y envío, en minutos.
...
Actualmente, el sistema HORUS trabaja cuatro tipos de imágenes:
...
En esta sección se describe la interfaz que sirve para configurar las
rutas de almacenamiento en disco de cada uno de estos tipos de
imágenes. En total, son ocho rutas que se deben configurar.
...
\item Se debe seleccionar la estación a la cual se le desea cambiar la
  ...
  donde se está haciendo el cambio o una dirección web. Si no cumple
  con esto la ruta se mostrará vacía.

  En las opciones de la (2) a la (9) se selecciona la nueva ruta ya
  sea reescribiéndola en el campo de texto o dando clic en el botón
  ...
  la lectura de las imágenes, si se desean guardar las imágenes
...
\item Botón para guardar la configuración en el archivo
  ...
  comprobación encuentra que algún campo no cumple con esto, lo
...
información en la base de datos de un ROI. Al abrir la interfaz, se
muestran los ROIs disponibles en la base de datos, los cuales pueden
ser actualizados. También es posible agregar un nuevo ROI.
...
\item Posiciones en $u$ si se escogió la opción de actualizar y hay
  información en la base de datos, por el contrario si se escogió
  insertar se mostrará vacío para que se ingrese la información. Éste
  es un valor numérico posiblemente con cifras decimales después del
  (.). El campo que se muestra al lado derecho es para visualizar la
  posición de los puntos cuando se está haciendo la marcación con
  (11). Deben ser mínimo tres puntos para poder así cerrar el
  polígono.
\item Posiciones en $v$ si se escogió la opción de actualizar y hay
  información en la base de datos, por el contrario si se escogió
  insertar se mostrará vacío para que se ingrese la información. Éste
  es un valor numérico posiblemente con cifras decimales después del
  (.). El campo que se muestra al lado derecho es para visualizar la
...
\item Botón para comenzar las marcaciones de los puntos $(u, v)$ sobre
  ...
  hacer con el botón <- que se encuentra en (14).
...
\item Este botón aparece cuando se ha ingresado la información como si
  ...
  datos. Si se desea insertar un nuevo ROI, este botón no estará
...
\item Opciones para hacer \textit{zoom}, desplazarse sobre la imagen y
  borrar el último vértice marcado sobre la imagen (<-). Se pueden
...
primera vez se deberá empezar a marcar el polígono desde el principio,
si la idea es agregar sólo un punto es recomendable hacerlo con (8) y
(9), o marcar encima de los puntos anteriores del polígono. Estas dos
...
La interfaz para la rectificación tiene como fin proveer al usuario
una manera de rectificar un conjunto de imágenes dados una estación,
una cámara, un tipo de imagen y un rango de tiempo. El usuario puede
definir un ROI en la imagen para la rectificación. Se presupone que ya
hay calibraciones almacenadas en la base de datos, de lo contrario el
usuario debe proceder a generar una con la interfaz
correspondiente. Por defecto, se escoge la última calibración
almacenada en la base de datos cuyo tiempo de generación sea el más
cercano antes del tiempo de cada imagen, sin embargo, es posible
escoger cualquier otra calibración para rectificar el conjunto
...
\item Selección de las cámaras y el orden entre ellas. En la lista
  ...
  ``$\ll$'' borra de la lista de cámaras cargadas una en particular.
...
La interfaz para la fusión tiene como fin proveer al usuario una
manera de configurar los criterios de búsqueda de un grupo de imágenes
y fusionarlas dados una estación, un tipo de imagen y un rango de
tiempo. El usuario tiene la opción de fusionar imágenes oblicuas,
fusionar imágenes ya rectificadas, o de rectificar y luego fusionar
las imágenes, para un conjunto de cámaras. Por defecto, se escoge la
última calibración almacenada en la base de datos cuyo tiempo de
generación sea el más cercano antes del tiempo de cada conjunto de
imágenes, sin embargo, es posible escoger cualquier otra calibración
para fusionar el conjunto completo de imágenes. En la
...
\item Lista desplegable donde se puede escoger alguno de los tipos de
  imágenes disponibles en la base de datos (e.g.\@\textit{Snap, Timex,
    Var}).
\item Si se escoge la opción ``Rectify \& merge images'' se buscan
  imágenes oblicuas que se rectifican y luego se fusionan. Si se
  escoge la opción ``Merge already rectified images'' se buscan
  imágenes ya rectificadas y se fusionan. Si no se elige ninguna
  opción, sólo se fusionan las imágenes oblicuas.
...
imágenes y un rango de fechas.  En esta interfaz cada vez que se haga
alguna consulta, se inserte, actualice o elimine información de la
base de datos se mostrará una ventana indicando esto y se cerrará
automáticamente cuando haya finalizado.  En las figuras~\ref{fig:min}
...
\begin{enumerate}
\item Lista para seleccionar la estación a la que se le quiere generar
  miniaturas.
...
\item Ruta donde se guardan las imágenes miniaturas, si se ha
  ...
  podrá editar a menos que lo haga usando la interfaz \textit{Path
    Editor}. Si es el caso, la ruta se puede cambiar ya sea
  escribiendo la nueva ruta en este campo o dando clic en el botón
  \textit{\ldots} que abrirá una nueva ventana para que se seleccione
  la ruta.
...
\item Ancho deseado de las nuevas imágenes, la cual es el número 
  de pixeles que se desea tenga el ancho de las imágenes. 
  Éste es un valor entero. Se recomienda que el ancho de 
  la miniatura sea de 120 píxeles.
...
\item Host del servidor puede ser una dirección IP o un nombre de dominio.
\item Se debe ingresar el usuario.
\item Se debe ingresar la contraseña.
\item Este botón es usado para guardar los datos de conexión. Solo se
...
rectificar, fusionar y miniaturizar imágenes manualmente. Sin embargo,
en general es deseable que estos procesos se hagan automáticamente. En
esta sección se describe la interfaz para configurar el automático de
procesamiento para rectificar, fusionar, miniaturizar y enviar a la
web las imágenes. La información de la configuración se guarda en el
...
\item Tamaño de paso entre procesamiento y procesamiento, en minutos.
\item Tipo de procesamiento a aplicar a las imágenes, las opciones son
  \textit{Merge only}, para fusionar imágenes oblicuas,
