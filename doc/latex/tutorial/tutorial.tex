...
fallos en la red o el internet es decir se pierde la conexión con la
base de datos se mostrara de nuevo la interfaz de \textit{login}.
...
\chapter{Instalar y configurar base de datos}
\label{chap:bd}
...
disponer del servidor de bases de datos MySQL. Para esto se deben
...
\item Instalar el servidor de MySQL siguiendo los pasos del instalador
  ...
  \item Por defecto, existe un usuario administrador del servidor. Se
    debe especificar la contraseña de este usuario (\verb!root!). Esta
...
estación, para ello se presiona el botón \textbf{Edit database}. Se
mostrará una ventana solicitando la información de la estación o si ha
...
embargo, para efectos del tutorial, este paso es necesario.

Como ejemplo, se insertarán las imágenes de prueba de la estación
CARTAGENA ubicadas en el directorio \emph{examples} del paquete
HORUS. Sabiendo esto, desde la línea de comandos se llamará a la
función \emph{insert\_images\_recursively} del directorio \emph{io},
que tiene como datos de entrada la ruta donde se encuentran las
imágenes y la estación asociada a ellas.
...
imágenes de HORUS. La estructura de carpetas se muestra en la
figura~\ref{fig:tutorial_insert_obli} y debe contener lo siguiente, en
forma anidada:

Un directorio que tendrá las estaciones, en este caso tiene el nombre
\texttt{dbimage}, pero no hay restricciones con el nombre.
...
puede usar la función \emph{load\_allimage} del directorio \emph{io}
...
\item Se escogen cada uno de los GCPs, y se presiona el botón
  ...
  evitar diferencias con los resultados de este tutorial.
...
llamado. La segunda forma es presionando el botón \textbf{Create new
  ROI} donde se debe diligenciar toda la información manualmente, como
se describe en el apéndice~\ref{app:roi}.

En la interfaz de creación ROI, parte de la información está completa,
por lo tanto se procede a presionar el botón \emph{Mark Points} para
marcar la región de interés en la imagen, haciendo clic sobre la
imagen en los puntos correspondientes a los vértices del polígono del
ROI. Se debe tener en cuenta que el último punto a marcar se marca con
clic derecho para cerrar el polígono del ROI. En la parte superior
...
calibración, se debe cerrar esta interfaz haciendo clic en la
\textbf{X} en la parte superior derecha. En la
...
coordenadas, éstas se graficarán sobre la imagen. Se sugiere que
cuando a la hora de realizar la calibración de cada cámara, se ingrese
esta información de coordenadas para obtener resultados similares a
los presentados en este tutorial.
...
se fusionarán las cámaras. Para el ejemplo se hará en el orden
C1-C2-C3, para ello se selecciona de la lista desplegable la cámara C1
y se presiona el botón $>>$ para agregarlo a la lista del cuadro del
lado derecho.  Luego, se repite el proceso para C2 y por último para
C3. Luego de esto, se presiona el botón \emph{Load images}, el cual
cargará las imágenes en pares con el orden que se indicó, en este caso
primero C1-C2 y luego
C2-C3.\\
...
\item Se debe tener en cuenta que al hacer la marcación se pueden
  ...
  de que se obtengan resultados similares.
...
procede selecciona de la lista desplegable la cámara C1 y se presiona
el botón $>>$ para agregarlo a la lista del cuadro del lado
derecho. Luego, se repite el proceso para C2 y por último para
C3. Luego de esto, se presiona el botón \emph{Load images}, el cual
cargará las imágenes en pares con el orden que se indicó, en este caso
...
lote. Para fusionar un grupo de imágenes, se presiona el botón
\textbf{Manually merge images} en la interfaz principal.

El proceso de fusión de imágenes en lote es muy similar al de
...
la captura es casi igual que en el caso de la captura de imágenes,
...
escoger una región de interés (ROI). Para el ejemplo, los valores
...
correspondientes a los vértices del polígono que conforma el ROI. Para
...
ser solicitada al equipo HORUS al correo electrónico:
...
de ingresar la información se debe presionar el botón \textit{Save
...
de subida. Luego de esto, se mostrará una barra de progreso con la
cual se puede cancelar la operación en cualquier momento.
...
altura de ola, dirección de oleaje entre otros. Esta configuración
se realiza desde la interfaz \textbf{Edit database}, presionando el
botón correspondiente en la interfaz principal. Los pasos son los
siguientes:
...
Ahora se explicará la segunda manera de utilizar la interfaz para
crear un ROI, es decir, presionando el botón \textbf{Create new ROI}
en la interfaz principal. Como ejemplo se ingresa la siguiente
