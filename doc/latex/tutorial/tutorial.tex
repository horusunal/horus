...
son: dirección del servidor (\emph{Database host}), nombre de la base
de datos (\emph{Database name}), puerto de la base de datos en el
servidor (\emph{Database port}), nombre de usuario del administrador
de la base de datos de HORUS (\emph{Admin username}, no es el usuario
\emph{root} de la instalación de MySQL), contraseña del usuario
administrador (\emph{Admin password}) y la ruta del archivo con la
estructura de la base de datos de HORUS (\emph{Database SQL
  file}). Para más información, diríjase al manual de HORUS, Capítulo
\emph{Instalación}, Sección \emph{Base de Datos}. Para el ejemplo, los
campos rellenados son:
...
el botón \emph{Save ROI} o en la \textbf{X} en la parte superior
derecha. En la figura~\ref{fig:tutorial_roi} se muestra la interfaz
diligenciada para la cámara C1.
...
nuevo se puede hacer clic en este mismo menú en \textit{Show rectified
  image with/without grid}.  Para el ejemplo, se deben guardar las
calibraciones de las cámaras C1, C2, C3 con los errores antes
mencionados. En las
...
\item \emph{Camera}: C1
\item \emph{Select a calibration}: \emph{Default (nearest before time)}
...
\item \emph{Station}: CARTAGENA
\item \emph{Image type}: snap
\item Seleccionar la opción \emph{Merge oblique images}.
\item \emph{Select a fusion}: \emph{Default (nearest before time)}.
\item \emph{Initial time}: 01/05/2010 10:45:00
\item \emph{Final time}: 01/05/2010 22:45:00
\item \emph{Time error}: 3
\item \emph{Time step}: 30
...
búsqueda de las imágenes que se desean procesar. El tamaño de paso se
refiere al tiempo entre imágenes consecutivas, por ejemplo, si es 30
minutos, y el tiempo de la primera imagen es 06:00, entonces las
imágenes se buscan en los tiempos 06:00, 06:30, 07:00, etc. El error
se refiere a la desviación con respecto al tiempo en el que se espera
encontrar una imagen, por ejemplo, si el error es de 3 minutos, y el
tiempo de búsqueda es 06:30, entonces cualquier imagen entre 06:27 y
06:33 se puede considerar. Siempre se escoge la imagen con tiempo más
cercano al tiempo de búsqueda.
...
\item \emph{Z}: 0
\item \emph{Is this a virtual sensor?}: \emph{No}. Este campo se
  refiere a si el sensor es virtual, es decir, si es un modelo
  computacional que genera mediciones.
\item \emph{Description}: Este sensor reporta datos de marea
  correspondientes a la bahía de Cartagena.
\end{enumerate}

Si se tiene un archivo con mediciones para un sensor, se pueden
agregar automáticamente presionando el botón \emph{Import sensor
  data}.
...
de datos. Para confirmar la inserción, presionar el botón \emph{Yes}
en la ventana de confirmación. Esta inserción puede tardar unos
