function [varargout] = insert_calibration(conn, camera, station, timestamp,...
...
try
    ...
    try
        newid  = generate_autoinc(conn, ['calibration_' lower(station)], ...
            'idcalibration', station);
        % Data for insertion in calibration
        colnames = {'idcalibration', 'camera', 'station', 'timestamp', 'resolution'};
        extdata = {newid, camera, station, timestamp, resolution};
    ...
    for i = 1:2:n
        ...
        insert_calibrationparameter(conn, station, newid, name, value);
    end
