    ...
    try
        for i=1:length(type)
            query = ['SELECT filename, path, timestamp, type, mi.idfusion '...
                'FROM image_' station ' i NATURAL JOIN mergedimage_' station ' mi '...
                ...
                ' AND mi.idfusion IN ('...
                'SELECT DISTINCT f.id FROM fusion_' station ' AS f INNER JOIN'...
                ' camerabyfusion_' station ' AS cf ON f.id = cf.idfusion'...
