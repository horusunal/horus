function  data  = load_datemax(conn, datat, station, isthumb)

%LOAD_DATEMAX this function is used for querying the time of the last
%inserted image.
%
%   Input:
...
%
%   Output:
%   data: Time of the last image.
%
%   Example:
%   data  = load_datemax(conn, 'oblique', 'CARTAGENA', 0 )
%
...
try
    ...
    try
        switch datat
            case 'oblique'
                query = ['SELECT timestamp FROM image_' station ' NATURAL JOIN '...
                    'obliqueimage_' station ' WHERE ismini = ' num2str(isthumb) ' AND '...
            ...
            case 'rectified'
                query = ['SELECT i.timestamp FROM image_' station ' AS i NATURAL JOIN '...
                    'rectifiedimage_' station ' AS ri JOIN calibration_' station ' AS c WHERE ismini = ' num2str(isthumb) ' AND '...
            ...
            case 'merge_rectified'
                query = ['SELECT i.timestamp FROM image_' station ' AS i NATURAL '...
                    'JOIN mergedimage_' station ' AS mi JOIN fusion_' station ' AS f JOIN camerabyfusion_' station ' AS cf '...
                    'WHERE mi.idfusion=f.id AND '...
                    'f.id=cf.idfusion AND ismini = ' num2str(isthumb) ' AND '...
