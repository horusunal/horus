function  data  = load_image_step(conn, type, camera, station, time, error)

%LOAD_IMAGE_STEP   Finds a group of images whose timestamp
...
%          {filename, path, timestamp, type, camera}
%
%   Example:
%   data  = load_image_step(conn, {'snap', 'timex'}, 'C1', 'CARTAGENA', 734229:734240, 1/24 );
%
...
try
    ...
    try
        for t = 1:length(time)
            for i=1:length(type)
                query = ['SELECT filename, path, timestamp, type, camera '...
                    'FROM image_' station ' i NATURAL JOIN obliqueimage_' station ' oi '...
                    'WHERE ismini = 0 '...
                    'AND type IN ( '...
                    'SELECT idtype '...
                    ...
                    'AND timestamp BETWEEN ' num2str(time(t)-error-EPS,17) ' AND ' num2str(time(t)+error+EPS,17) ' '...
