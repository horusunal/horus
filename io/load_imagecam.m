...
%LOAD_IMAGECAM this function is used for querying images to merged.
%
%   Input:
...
try
    ...
    for i=1:length(timestamp)
        ...
        try %% Cambiar timestamp ya que puede no ser exacto
            ...
            query_numcam = ['SELECT COUNT(camera) FROM camerabyfusion_' station ' WHERE station LIKE "' ...
                station '" AND idfusion = (SELECT id FROM fusion_' station ' WHERE type LIKE "' ...
            ...
            for j=1:length(type)
                ...
                if strcmpi(typefusion, 'oblique')
                    query = ['SELECT DISTINCT filename, path, timestamp, camera '...
                        ...
                        'FROM image_' station ' NATURAL JOIN obliqueimage_' station ' JOIN ( '...
                        'SELECT camera as camerafusion, sequence '...
                        'FROM camerabyfusion_' station ' '...
                    ...
                    query = ['SELECT DISTINCT filename, path, timestamp, camera '...
                        'FROM (SELECT DISTINCT filename, path, timestamp, camera, cameraorder.sequence AS sequence '...
                        'FROM image_' station ' NATURAL JOIN (SELECT filename, camera, station '...
                        'FROM rectifiedimage_' station ' JOIN calibration_' station ' '...
