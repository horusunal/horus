...
try
    ...
    try
        switch imgtype
            case 'oblique'
                query = ['SELECT MAX(timestamp) FROM image_' lower(station) ' NATURAL JOIN '...
                    'obliqueimage_' lower(station) ' WHERE ismini = 1 AND '...
            ...
            case 'merge_oblique'
                query = ['SELECT MAX(i.timestamp) FROM image_' lower(station) ' AS i NATURAL JOIN '...
